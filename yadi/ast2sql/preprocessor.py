...
class ConjunctiveQueryPreprocessor():
    ...
    ''' This method takes a query, makes a deep copy of it and modifies it to \
        keep only equality constraints of the form:
        Var = const if Var occurs in the head OR
        const = const
        ie, constraints of the form Var1 = Var2, const = Var, Var = Const
        (if Var does not occur in the head) are eliminated.
        ...
        The only way to decide so is by noticing that because X=Y and X=2,
        Y must also be 2.
        This can be done if the equalities are reduced according to what
        was said previously:
    ...
    def reduce_equality_constraints(self,query):
        ...
        equality_constraints = [x for x in query.constraints \
            if x.is_equality_constraint()]
            ...
            if set1== -1 and set2== -1: # Neither El1 nor El2 is in a set.
                ...
                if set1>= 0:
                    if set2 == -1: # El1 is in some set and El2 is not.
                        ...
                        if set1!=set2: # If they're not in the same set,
                                       # merge them and delete one of them.
        ...
        for s in eq_sets:
            ...
            vars_in_rels = set([y for x in query.relations \
                                  for y in x.variables if y in variables])
            # Need variables in all relations, as well as vars in equality constraints.
            # Unioning with get_safe_variables() includes those variables in
            # equality constraints
            variables_occur_relation = list(vars_in_rels | \
                set(self.get_safe_variables(query, query.get_var_dict())))

            if len(variables_occur_relation)==0 and len(variables)>0 :
                raise NotInstantiatedException('Equivalence set of ' + \
                    str(variables[0]) + \
                    ' does not unify with a variable in a positive goal' )
            ...
            if len(constants)>0:
                ...
                # Substitute every occurence of the variable in the constraints
                # with the constant
                ...
                # Make sure we can still unify head variables with constants if
                # they are in the equivalence set.
                for variable in variables:
                    if variable in query.head_relation.get_variables():
                        new_constr = \
                            Constraint(variable, constant, Constraint.EQ_OPERATOR)
                        new_eq_constraints.append(new_constr)
            else:
                # Substitute every occurence of variable in the relations with
                # one variable from variables_occur_relation
                ...
                # Substitute every occurence of variable in the head with one
                # variable from variables_occur_relation
                ...
                # Substitute every occurence of variable in the constraints with
                # one variable from variables_occur_relation
                for constraint in query.constraints:
                    el0 = constraint.get_left_side()
                    el1 = constraint.get_right_side()
            ...
            # If there's more than one constant in this equivalence set, they are
            # all different so we need to keep one of these constraints so the
            # query returns false.
            if len(constants)>1:
                new_constr = Constraint(constants[0], constants[1], Constraint.EQ_OPERATOR)
                new_eq_constraints.append(new_constr)

        query.constraints = [x for x in query.constraints \
                                if not x.is_equality_constraint()\
                            ] + new_eq_constraints
        return query
